...
def fig11_leakage(leaky=None, honest=None, name="Fig11_leakage_effect",
    ...
    fig.subplots_adjust(left=0.07, right=0.985, top=0.72, bottom=0.20,
                        wspace=0.34)
    for ax, k, u in zip(axes, keys, units):
        vals = [leaky[k], honest[k]]
        bars = ax.bar([0, 1], vals, width=0.56,
                      color=[CRITICAL, BLUE], zorder=3)
        for b, v in zip(bars, vals):
            ax.text(b.get_x() + b.get_width() / 2, v, f"{v:.2f}",
                    ha="center", va="bottom", fontsize=7.6, color=INK,
                    fontweight="bold")
        ax.set_xticks([0, 1])
        ax.set_xticklabels(["leaky\nprotocol", "causal\nprotocol"],
                           fontsize=7.2)
        tidy(ax)
    ...
    infl = (honest["MAPE"] - leaky["MAPE"]) / honest["MAPE"] * 100
    fig.suptitle("The same learner, the same splits — only the decomposition "
                 "protocol differs", x=0.07, ha="left", y=0.965, fontsize=9.4,
                 fontweight="bold")
    fig.text(0.07, 0.855, f"decompose-then-split reports {infl:.1f} % lower "
             f"error than is attainable causally: the gain is an artefact of "
             f"the protocol, not of the model", fontsize=7.2, color=CRITICAL)
    save(fig, os.path.join(FIGURES_DIR, name))
